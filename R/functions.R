#' CLix Scoring Function
#'
#' This function will score and assign an AD subtype each case based on the distribution it's tangles
#' @param dat the data containing cases to assign a score and AD subtype
#' @param reference.dat the reference data containing cases for which to score cases against, if NA nbM Flame cohort is used
#' @param idname the unique identifier for which to identify the cases, default is NPID
#' @param posHC.dat default is NA
#' @param posHC.refdat default is NA
#' @param crit.values default is TRUE
#' @keywords hpsp
#' @export
HpSp.scoring <- function(dat = NA,
                         reference.dat = NA,
                         idname='NPID',
                         posHC.dat=NA,
                         posHC.refdat=NA,
  ...
  if(n.with.missing==0){
    res<-HpSp.scoring.nomissing(dat,refdat,idname=idname,crit.values=crit.values)
  }else{
    cat('No. of cases with missing data is',n.with.missing,'of',length(ind),'\n')
    dat$HpSp.score.subtype <- rep(NA,n)
    dat$HpSp.score <- rep(NA,n)
    dat$score.lo <- rep(NA,n)
    dat$score.hi <- rep(NA,n)
    # first find the minimum possible score (most HpSp) the case could have:
    dat.lo<-dat
    ...
    res.lo <- HpSp.scoring.nomissing(dat.lo,refdat,idname=idname,crit.values=crit.values)
    scores.lo<-res.lo$HpSp.score[is.na(dat$R)]
    ...
    res.hi<- HpSp.scoring.nomissing(dat.hi,refdat,idname=idname,crit.values=F)
    scores.hi<-res.hi$HpSp.score[is.na(dat$R)]
    ...
    res.impute<-HpSp.scoring.nomissing(dat.impute,refdat,idname=idname,crit.values=F)
    ...
    res$HpSp.score.subtype[ind.allmissing]<-'No data'
...
HpSp.scoring.nomissing <- function(dat,
                                   reference.dat,
                                   idname='NPID',
                                   details=F,
                                   crit.values=T){
  ...
  dat$kstar[ind]<- apply(cbind(dat$kR,dat$kH,dat$kC)[ind,],1,max) # max of all these three is the final percent!  values of 25 or less are HpSp
  ...
  dat$kC[ind] <- 50
  dat$kstar[ind]<-50
  dat$HpSp.score <- 0.4*dat$kstar
  dat$HpSp.score.subtype[dat$HpSp.score<10]<-'HpSp'
  dat$HpSp.score.subtype[(dat$HpSp.score>=10)&(dat$HpSp.score<30)]<-'Typical'
  dat$HpSp.score.subtype[dat$HpSp.score>=30]<-'Limbic'
  dat$HpSp.score.subtype[is.na(dat$R)]<- 'Missing Data'
  if(details){res<-dat}else{
    res<-dat[,match(c(idname,'R','H1','H2','H3','C1','C2','C3','C4','HpSp.score.subtype','HpSp.score'),names(dat))]
  }
  res
}
